...
if (have_posts()) {
	...
	if (!$sticky_out) {
		if (good_wine_shop_get_theme_option('first_post_large') && !is_paged() && !in_array(good_wine_shop_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
	...
	while ( have_posts() ) { the_post(); 
		...
		get_template_part( 'content', $sticky_out && is_sticky() ? 'sticky' :'classic' );
	}
