...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	class="sc_icons sc_icons_modern sc_icons_size_<?php echo esc_attr($args['size']); ?><?php if (!empty($args['align'])) echo ' sc_align_'.esc_attr($args['align']); ?><?php if (!empty($args['class'])) echo ' '.esc_attr($args['class']); ?>"
	...
	foreach ($args['icons'] as $item) {
		...
		?><div class="sc_icons_item<?php echo !empty($item['link']) ? ' sc_icons_item_linked' : ''; ?>"><?php
			if (!empty($item['image'])) {
				...
				if (!empty($icon)) {
					$svg = '';
					if ($args['icons_animation'] > 0 && ($svg = trx_addons_get_file_dir('css/svg-icons/'.trx_addons_esc($icon).'.svg')) != '') {
						$item['icon_type'] = 'svg';
						$svg_present = true;
