...
if ( !function_exists( 'trx_addons_sc_googlemap_merge_styles' ) ) {
	...
	function trx_addons_sc_googlemap_merge_styles($css) {
		return $css . file_get_contents(trx_addons_get_file_dir('shortcodes/googlemap/googlemap.css'));
	}
}
...
if ( !function_exists( 'trx_addons_sc_googlemap_merge_scripts' ) ) {
	...
	function trx_addons_sc_googlemap_merge_scripts($js) {
        if (trx_addons_get_option('api_google') != '') {
            trx_addons_enqueue_googlemap();
            if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
                $js .= file_get_contents(trx_addons_get_file_dir('shortcodes/googlemap/googlemap.js'));
            }
        }
        return $js;
	}
...
// Add messages for JS
if ( !function_exists( 'trx_addons_sc_googlemap_add_js_storage' ) ) {
	add_filter("trx_addons_js_storage", 'trx_addons_sc_googlemap_add_js_storage');
	function trx_addons_sc_googlemap_add_js_storage($storage) {
...
if ( !function_exists( 'trx_addons_sc_googlemap' ) ) {
	function trx_addons_sc_googlemap($atts, $content=null){	
        ...
        if (trx_addons_get_option('api_google') != '') {
            trx_addons_enqueue_googlemap();
            if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
                trx_addons_enqueue_script('googlemap', trx_addons_get_protocol() . '://maps.google.com/maps/api/js' . ($api_key ? '?key=' . $api_key : ''), array(), null, true);
                if (trx_addons_is_on(trx_addons_get_option('debug_mode')))
		...
		vc_map( apply_filters('trx_addons_sc_map', array(
				...
				"params" => array(
					...
					array(
						...
						"admin_label" => true,
						"class" => "",
						"value" => apply_filters('trx_addons_sc_googlemap_styles', array(
							esc_html__('Default', 'trx_addons') => 'default',
