...
if ( !post_password_required() && !apply_filters('trx_addons_filter_featured_image', false, $trx_addons_args) ) {
	...
	if ( $trx_addons_args['singular'] ) {
		...
		if ( has_post_thumbnail() ) {
			?><div class="post_featured<?php echo !empty($trx_addons_args['thumb_bg']) ? ' post_featured_bg' : '';  ?>">
				<?php 
				do_action('trx_addons_action_before_featured');
				?>
			...
			if ( ($trx_addons_output = trx_addons_build_slider_layout(array('thumb_size' => $trx_addons_thumb_size))) != '' ) {
				?><div class="post_featured"><?php echo trim($trx_addons_output); ?><?php
				echo trim($trx_addons_post_info);
				?></div><?php
			}
			...
			if (!empty($trx_addons_video)) {
				?><div class="post_featured"><?php echo trim($trx_addons_video); ?><?php echo trim($trx_addons_post_info); ?></div><?php
			}
			...
			if (empty($trx_addons_audio))
				$trx_addons_audio = trx_addons_get_post_iframe('', false);
			if (!empty($trx_addons_audio)) {
				?><div class="post_featured"><?php echo trim($trx_addons_audio); ?><?php echo trim($trx_addons_post_info); ?></div><?php
			}
		}
