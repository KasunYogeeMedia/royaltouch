...
if (!function_exists('trx_addons_build_slider_layout')) {
	function trx_addons_build_slider_layout($args=array(), $images=array()) {
		...
		if (empty($args['thumb_size']))
			$args['thumb_size'] = trx_addons_get_thumb_size( $args['height'] >= 630
																	? 'full'
																	: ( $args['height'] >= 420
																		? 'huge'
																		: ( $args['height'] >= 210
			...
			$output .= '<div' . (!empty($args['id']) ? ' id="' . esc_attr($args['id']) . '_outer"' : '')
					...
					. '<div' . (!empty($args['id']) ? ' id="' . esc_attr($args['id']) . '_swiper"' : '')
						. ' class="slider_swiper swiper-slider-container' . (!empty($args['class']) ? ' '.esc_attr($args['class']) : '')
							...
							. ((int)$args['height']==0 ? ' slider_noresize' : '')
							.'"'
			...
			foreach ($images as $image) {
				...
				$output .= '<div ' . (!empty($image['id']) ? ' id="' . esc_attr($image['id']) . '"' : '') 
								...
								. ' style="'
									...
									. '"'
								. '>'
								. ($args['slides_type'] == 'bg' || empty($image['url']) ? '' : '<img src="' . esc_url($image['url']) . '" alt="'.esc_attr($image['title']).'">')
								. ($video_embed || $video_link
									...
									: (!empty($titles) && $args['titles']!='outside'
											...
											: ($image['link'] ? '<a href="'.esc_url($image['link']).'" class="slide_link"></a>' : '')
											)
									)
								. (!empty($image['content']) 
