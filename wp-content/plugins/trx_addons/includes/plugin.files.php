...
if (!function_exists('trx_addons_is_on')) {
    function trx_addons_get_theme_option($prm) {
        if (function_exists('themerex_get_theme_option')) {
            return trx_addons_get_theme_option($prm);
        }
        if (function_exists('good_wine_shop_get_theme_option')) {
            return good_wine_shop_get_theme_option($prm);
        }
        return null;
    }
}
...
if (!function_exists('trx_addons_check_min_file')) {	
	function trx_addons_check_min_file($file, $dir) {
		if (substr($file, -3)=='.js') {
			if (substr($file, -7)!='.min.js' && trx_addons_is_off(trx_addons_get_option('debug_mode'))) {
			...
			if (substr($file, -8)!='.min.css'  && trx_addons_is_off(trx_addons_get_option('debug_mode'))) {
				$dir = trailingslashit($dir);
