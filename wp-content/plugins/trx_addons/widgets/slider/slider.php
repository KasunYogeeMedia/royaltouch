...
class trx_addons_widget_slider extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ($engine == 'swiper') {
			...
			$height = isset($instance['height']) ? (int) $instance['height'] : 0;
...
if ( !function_exists( 'trx_addons_sc_slider_controller' ) ) {
	function trx_addons_sc_slider_controller($atts, $content=null){	
		...
		$output = '<div' . ($atts['id'] ? ' id="'.esc_attr($atts['id']).'"' : '')
						. ' class="sc_slider_controller sc_slider_controller_'.esc_attr($atts['controller_style']) 
							. (!empty($atts['class']) ? ' ' . esc_attr($atts['class']) : '') 
							. '"'
		...
		$params = array_merge($params, array(		
					array(
						...
						"value" => array(
                                    ...
                                    esc_html__('Special', 'trx_addons') => 'special'
									),
						"type" => "dropdown"
					),
					array(
				...
				"params" => array(
					...
					array(
						...
						"value" => array(
                                    ...
                                    esc_html__('Products', 'trx_addons') => 'products'
									),
						"type" => "dropdown"
					),
					array(
